...
<h1>Editar filtro - {{$card->titulo}}</h1>
@stop
    ...
    <div class="float-left w-3/4">
        <div class="p-4 overflow-x-auto bg-white rounded-lg shadow-md">
            <form id="cardsForm" class="space-y-4">
                @csrf
                <input type="hidden" value="{{$card->id}}" name="id" id="id">
                <div>
                    <label for="">Ruta laravel</label>
                    <input type="text" name="url" id="url" value="{{$card->url}}"
                ...
                <div>
                    <select name="int_ext" id="int_ext" class="w-full px-3 py-2 border border-gray-400 rounded-md">
                        <option value="interna" {{ $card->int_ext == 'interna' ? 'selected' : '' }}>Url Interna</option>
                        <option value="externa" {{ $card->int_ext == 'externa' ? 'selected' : '' }}>Url Externa</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="alt_title" id="alt_title" value="{{$card->alt_title}}"
                ...
                <div>
                    ...
                    <input type="text" name="imagen" id="imagen" value="{{$card->imagen}}"
                ...
                <div>
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{$card->titulo}}"
                ...
                <div>
                    ...
                    <input type="text" name="parrafo" id="parrafo" value="{{$card->parrafo}}"
                ...
                <div>
                    ...
                    <input type="text" name="boton" id="boton" value="{{$card->boton}}"
                ...
                <div>
                    ...
                    <select name="activo" id="activo" class="w-full px-3 py-2 border border-gray-400 rounded-md">
                        <option value="S" {{ $card->activo == 'S' ? 'selected' : '' }}>Si</option>
                        <option value="N" {{ $card->activo == 'N' ? 'selected' : '' }}>No</option>
                    </select>
...
<script>
    document.getElementById('cardsForm').addEventListener('submit', function(event) {
        ...
        axios.post(`/admin/ActualizarHomeCards/${id}`, formData)
            .then(response => {
