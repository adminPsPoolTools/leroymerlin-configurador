...
<div class="float-left w-full mb-5 ">
    <a class="text-primary tamaño-logos mb-1 mr-1" href="{{ route('admin.cloradores.create') }}">Nuevo Clorador <i
            class='fas fa-plus-circle'></i></a>

    <div class="grid grid-cols-1 gap-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5">
        @foreach ($modelos as $modelo)
        @if ($cloradoresPorModelo->has($modelo->id))
        <div class="col-span-1 overflow-hidden bg-white rounded-lg shadow-md">
            ...
            <table class="min-w-full table-auto">
                ...
                <tbody>
                    @foreach ($cloradoresPorModelo[$modelo->id] as $clorador)
                    <tr>
                        <td class="px-2 py-1">{{ $clorador->codigo }}</td>
                        <td class="px-2 py-1">{{ $clorador->descripcion }}</td>
                        <td class="px-2 py-1">{{ $clorador->valor }}</td>
                        <td class="px-2 py-1">
                            @if (empty($clorador->url))
                            No
                            @else
                            <a target="_blank" href="{{ $clorador->url }}">Url</a>
                            @endif
                        </td>
                        <td class="w-1/12 px-2 py-1 align-middle">
                            <div class="flex justify-center">
                                <a class="text-primary tamaño-logos mb-1 mr-1"
                                    href="{{ route('admin.cloradores.edit', $clorador->id) }}">
                                    <i class='fas fa-edit'></i>
                                </a>
                                <form action="{{ route('admin.cloradores.delete', $clorador->id) }}" method="GET"
