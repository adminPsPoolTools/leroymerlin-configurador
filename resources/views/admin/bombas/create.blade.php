    ...
    <div class="w-2/4 float-left">
        <div class="overflow-x-auto p-4 bg-white shadow-md rounded-lg">
            <form id="bombasForm" class="space-y-4">
                @csrf
                <div>
                    <input type="text" name="codigo" id="codigo" value="" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Código artículo">
                    <span class="text-red-500 text-sm" id="error-codigo"></span>
                </div>
                <div>
                    <input type="text" name="descripcion" id="descripcion" value="" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Descripción">
                ...
                <div>
                    <input type="text" name="caudal" id="caudal" value="" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Caudal a 10 mca">
                    <span class="text-red-500 text-sm" id="error-caudal"></span>
                </div>
                <div>
                    <input type="text" name="url" id="url" value="" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Url web">
...
<script>
    document.getElementById('bombasForm').addEventListener('submit', function(event) {
        ...
        axios.post('{{ route('admin.bombas.store') }}', formData)
            .then(response => {
            ...
            .catch(error => {
                if (error.response && error.response.data.errors) {
                    ...
                    if (errors.caudal) {
                        document.getElementById('error-caudal').textContent = errors.caudal[0];
                    }
                    if (errors.url) {
