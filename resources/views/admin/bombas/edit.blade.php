...
@section('content_header')
    <h1>Editar bomba  - {{$bombas->descripcion}}</h1>
@stop
...
<div class="w-2/4 float-left">
    <div class="w-2/4 float-left">
        <div class="overflow-x-auto p-4 bg-white shadow-md rounded-lg">
            <form id="bombasForm" class="space-y-4">
                @csrf
                <input type="hidden" value="{{$bombas->id}}" name="id" id="id">
                <div>
                    <input type="text" name="codigo" id="codigo" value="{{$bombas->codigo}}" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Código artículo">
                    <span class="text-red-500 text-sm" id="error-codigo"></span>
                </div>
                <div>
                    <input type="text" name="descripcion" id="descripcion" value="{{$bombas->descripcion}}" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Descripción">
                ...
                <div>
                    <select name="fk_modelo" id="fk_modelo" class="border border-gray-400 rounded-md px-3 py-2 w-full">
                        @foreach ($modelos as $modelo)
                            <option value="{{ $modelo->id }}" {{ $bombas->fk_modelo == $modelo->id ? 'selected' : '' }}>{{ $modelo->descripcion }}</option>
                        @endforeach
                    </select>
                    <span class="text-red-500 text-sm" id="error-fk_modelo"></span>
                </div>
                <div>
                    <input type="text" name="caudal" id="caudal" value="{{$bombas->caudal}}" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Caudal Bomba">
                    <span class="text-red-500 text-sm" id="error-caudal"></span>
                </div>
                <div>
                    <input type="text" name="url" id="url" value="{{$bombas->url}}" class="border border-gray-400 rounded-md px-3 py-2 w-full" placeholder="Url web">
...
<script>
    document.getElementById('bombasForm').addEventListener('submit', function(event) {
        ...
        document.getElementById('error-caudal').textContent = '';
        ...
        axios.post(`/admin/ActualizarBomba/${id}`, formData)
            .then(response => {
            ...
            .catch(error => {
                if (error.response && error.response.data.errors) {
                    ...
                    if (errors.caudal) {
                        document.getElementById('error-caudal').textContent = errors.diametro[0];
                    }
                    if (errors.url) {
