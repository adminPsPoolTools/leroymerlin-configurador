...
switch ($tipo) {
    ...
    case 2: // NO SE USA Cambia el cliente del presupuesto
    ...
    case 3: // NO SE USA Imprimir el presupuesto
    ...
    case 4: // Enviar correo del presupuesto(no funciona)
    ...
    case 5: // Calculamos presupuestos con descuento cliente.
...
function calcularYGrabarPresupuesto($datos, $conn)
{
    ...
    if (!empty($datosMotor)) //SI ESTÁ VACIO ES QUE NO HAY UN MOTOR CON ESAS CARACTERISTICAS DISPONIBLE
    {
        ...
        if (((float) $largo) >= ((float) $largoTotal)) // SI LA LONGITUD TOTAL ES MENOR O IGUAL A LA MAXIMA, ES VALIDO
        {
            ...
            if ($datos->conViga) {
                insertarVigas($conn, $datos->subtipo, $nuevoCodigo, $linea, $datos->anchoPiscina, $datos->cliente, $largoTotal, $transacciones);
                ...
                if ($datos->tipoTapa == "revestir") {
                    ...
                    insertarAnclajesVigaRefuerzoDeck($conn, $datos, $nuevoCodigo, $linea, $transacciones, $datos->anchoPiscina, $largoTotal, 'ANCLAJE VIGA REFUERZO', 1, 'Articulo fijo');
                }
            ...
            insertarPatines($conn, $datos->modelo, $datos->subtipo, $nuevoCodigo, $linea, $anchoTotal, $datos->cliente, $transacciones);

            //Insertar tubo salida para CAVE
            insertarTuboSalida($conn, $datos->modelo, $datos->subtipo, $nuevoCodigo, $linea, $anchoTotal, $datos->cliente, $transacciones);
            ...
            $importeTotal = mysqli_fetch_object($mresPrecio);

            $sqlImportePVP = "SELECT SUM(CANTIDAD * PVP) AS TOTAL FROM LINPRESU WHERE PRESUPUESTO_WEB = '$nuevoCodigo'";
            $mresPrecioPVP = mysqli_query($conn, $sqlImportePVP);
            $importeTotalPVP = mysqli_fetch_object($mresPrecioPVP); // oJO MULTIPLICAR CANTIDAD POR PVP
            ...
            $sql = "INSERT INTO PRESUPUESTOS (TITULO, DESCRIPCION, CLIENTE, FECHA, TOTAL, CODIGO_WEB, IMPUESTO, COMENTARIO_INTERNO, DESCRIPCION_CLIENTE)
                    VALUES ('" . utf8_decode($descripcionCliente) . "','$descripcion', $datos->cliente, NOW(), {$importeTotal->TOTAL}, $nuevoCodigo, 21,
            ...
            $result = array(
                ...
                "PRECIO"    => round(($importeTotal->TOTAL * 1.21), 2),
                "PRECIO_PVP"    => round(($importeTotalPVP->TOTAL * 1.21), 2),
